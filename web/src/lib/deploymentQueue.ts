 ...
 *
 * Отказ 401/403 — это отказ не расстановке, а тому, кто её отправляет (токен
 * истёк за двенадцатичасовую смену без связи). Очередь остаётся целой и
 * уходит после повторного входа. Окончательны только 404/409/422 для батча и
...
export type FlushOutcome = {
  ...
  retryReason?: string;
  /** Сервер не принял учётную запись (401/403) или её сменили: очередь цела
...
async function runFlush(owner: string, calloutId: number): Promise<FlushOutcome> {
  ...
  try {
    ...
    if (status === 401 || status === 403) {
      // Токен истёк за долгую смену без связи или прав у этой сессии нет.
      // Отказ не про расстановку, а про того, кто её отправляет: очередь
      // остаётся целой и уйдёт после повторного входа.
