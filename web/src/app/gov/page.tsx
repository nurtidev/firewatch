...
const TRUST: { icon: LucideIcon; title: string; text: string }[] = [
  ...
  {
    icon: Lock,
    title: "Готово к размещению в РК",
    text: "Разворачивается на инфраструктуре заказчика в Казахстане — on-prem или облако РК; доступ разграничен по ролям и районам.",
  },
  {
    icon: BarChart3,
...
const AKIMAT_PROBLEMS: { icon: LucideIcon; title: string; text: string }[] = [
  ...
  {
    icon: ClipboardList,
    title: "Отчёт акиму и в маслихат — вручную из писем ДЧС",
...
const AKIMAT_SCREENS: {
  ...
  },
  {
    ...
    pain: "Что показать акиму и в маслихат?",
  ...
  return (
    <div className="min-h-screen bg-bg text-fg">
          ...
          {STAKES.map((s, i) => (
            <Reveal key={s.n} delay={i * 70}>
              <div className="flex flex-col">
                ...
                <p className="mt-3.5 text-[13.5px] leading-relaxed text-muted">{t(s.label)}</p>
            ...
            {PILOT.map((s, i) => (
              <Reveal key={s.week} delay={i * 70}>
                <div className="relative">
                  {/* Numbered node with a connecting rule — a stepped timeline, no boxes. */}
                  <div className="flex items-center gap-3">
      ...
      <Section id="trust">
        ...
        <Reveal>
          <div className="mx-auto mt-12 max-w-[820px] divide-y divide-border overflow-hidden rounded-xl border border-border bg-surface shadow-card">
            {TRUST.map((item) => (
              <div key={item.title} className="flex items-start gap-4 p-5 sm:gap-5 sm:p-6">
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-lg border border-border bg-surface-2 text-accent">
                  <item.icon className="h-5 w-5" strokeWidth={1.9} />
                </span>
                <div>
                  <h3 className="text-[16px] font-bold tracking-tight">{t(item.title)}</h3>
                  <p className="mt-1 text-[14px] leading-relaxed text-muted">{t(item.text)}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <Section id="twin">
          <Reveal>
            <div className="grid items-center gap-10 lg:grid-cols-[.9fr_1.1fr] lg:gap-16 [&>div]:min-w-0">
              ...
              <div>
                <div className="relative h-[360px] overflow-hidden rounded-xl border border-border bg-surface shadow-pop sm:h-[420px]">
