...
def _write_model_card(metrics: dict) -> None:
    ...
    lines = [
        ...
        "",
        "## Назначение",
        "Оценка риск-приоритета здания (0–100) для приоритизации надзорных "
        "проверок ДЧС РК. Не заменяет инспектора — ранжирует объекты по риску. "
        "Помимо вероятности возгорания учитывает функциональную пожарную "
        "опасность объекта (массовое пребывание людей, маломобильные/дети/"
        "стационар, категория производства А–Д, газоснабжение), поэтому это "
        "приоритет надзора с учётом тяжести последствий, а не чистая вероятность "
