...
async def upload_card(
    ...
    card_id = db.execute(
        ...
        ),
        {
            ...
            "fp": str(path),
            "ex": _json(extracted),
            # Загруженный PDF не привязан ни к зданию, ни к точке — отнести его
            # к району по геометрии нечем, поэтому район фиксируем по
            # загрузившему (он резолвится из БД на каждый запрос, см.
            # auth.current_user). Карточки с привязкой к зданию получают район
            # здания (scripts/seed_districts.py, _card_scope_sql).
            "district": user.get("district"),
        },
    ).scalar()
