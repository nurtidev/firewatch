...
`computed_at` через реальный JOIN и подзапрос max(), которые мок не
исполняет.
...
def stations_and_isochrones():
    ...
    - «stale»   — изохрона есть, но старше самой свежей в таблице (строгое
      `computed_at < max(computed_at)`, без запаса — все части одного прохода
      делят один и тот же `computed_at`, см. _stations_stale_isochrones) —
      как если бы rebuild_coverage не смог пересчитать именно эту часть в
      последнем проходе и оставил прежнюю геометрию (см. item 3);
